...
// type DataPoint struct {// dawi add
//     Time  int `json:"time"`
//     Speed int `json:"speed"`
// }

type DataSpeed struct {// dawi add
    Speed  int `json:"speed"`
    Time int `json:"time"`
}

type DataPower struct {// dawi add
    Power  int `json:"power"`
    Time int `json:"time"`
}

var db *sql.DB
// var dataPoints []DataPoint  // dawi add
var dataSpeeds []DataSpeed  // dawi add
var dataPowers  []DataPower  // dawi add
...
func main() {
    ...
    // })
	//......................................
	// POST請求處理器，用於接收車速和時間數據
	r.POST("/speed", func(c *gin.Context) {
		var speed DataSpeed
		if err := c.BindJSON(&speed); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		}
		dataSpeeds = append(dataSpeeds, speed)
		c.JSON(http.StatusOK, gin.H{"message": "Speed received successfully"})
	})

	// GET請求處理器，返回所有車速和時間數據
	r.GET("/speed", func(c *gin.Context) {
        c.JSON(http.StatusOK, dataSpeeds)
    })
	// ......................................
	// POST請求處理器，用於接收 power 和時間數據
	r.POST("/power", func(c *gin.Context) {
		var power DataPower
		if err := c.BindJSON(&power); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		}
		dataPowers = append(dataPowers, power)
		c.JSON(http.StatusOK, gin.H{"message": "Speed received successfully"})
	})

	// GET請求處理器，返回所有power和時間數據
	r.GET("/power", func(c *gin.Context) {
        c.JSON(http.StatusOK, dataPowers)
    })
